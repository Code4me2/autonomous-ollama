...
func (s *Server) GenerateHandler(c *gin.Context) {
	...
	if !req.Raw {
		...
		if values.Messages != nil && values.Suffix == "" && req.Template == "" {
			...
			prompt, images, err = chatPrompt(c.Request.Context(), m, r.Tokenize, opts, values.Messages, nil, req.Think, genTruncate)
			if err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
				return
			}
...
func (s *Server) ChatHandler(c *gin.Context) {
	...
	// MCP integration: manager init, tool discovery, context injection
	mcpState, caps := setupMCPForChat(&req, m, caps)
	var mcpManager *MCPManager
	if mcpState != nil {
		mcpManager = mcpState.Manager
	}
	...
	go func() {
		defer close(ch)
		runMCPToolLoop(MCPToolLoopParams{
			Server:           s,
			Ctx:              c.Request.Context(),
			Runner:           r,
			Model:            m,
	...
	if req.Stream != nil && !*req.Stream {
		resp, err := collectNonStreamingMCP(ch, req)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
